...
if 'data' in locals() and data is not None:
    ...
    with main_tabs[0]:
        ...
        vis_tabs = st.tabs(["Visualisation Individuelle", "Découpage & Superposition", "Analyse Comparative"])
        ...
        #  Onglet pour le découpage et Superposition
        with vis_tabs[1]:
            
            st.subheader("Découpage & Superposition")
            # Création de 2 sous-onglets
            sub_vis_tabs = st.tabs(["Découpage", "Superposition"])
            
            # Sous-onglet "Découpage" avec sélection interactive via on_select
            with sub_vis_tabs[0]:
                ...
                overlay_columns = st.multiselect("Colonnes supplémentaires à superposer",
                ...
                # Création du graphique interactif avec sélection (on_select)
                fig = go.Figure(data=go.Scatter(
                    ...
                    hovertemplate="Index: %{customdata}<br>" + f"{date_column}: %{{x}}<br>{target_column}: %{{y}}<extra></extra>"
                ))
                fig.update_layout(
                ...
                if event and "selection" in event:
                    ...
                    if selected_points:
                        ...
                        st.info("Aucune sélection détectée. Sélectionnez des points en cliquant ou en faisant glisser sur le graphique.")
                else:
                    st.info("Effectuez une sélection sur le graphique (en mode Box ou Lasso).")
                ...
                if "selection_summary" in st.session_state and st.session_state.selection_summary:
                    ...
                    # Option pour supprimer des sélections
                    ...
                    if selections_to_delete and st.button("Supprimer les sélections"):
                        ...
                        # 2) Supprimer les lignes correspondantes dans selected_data
                        ...
                        # 3) Forcer le rerun pour rafraîchir l'affichage
            ...
            # Sous-onglet "Superposition"
    
            with sub_vis_tabs[1]:
                st.header("Superposition des segments extraits (alignement à t=0)")
                
                # Plutôt que d'arrêter le script, on affiche un avertissement si aucune découpe n'est validée
                ...
                if selected_batches:
                    ...
                    for i, batch in enumerate(selected_batches):
                        ...
                        # Calcul de l'axe temporel relatif
                    ...
                    if len(X_valid) > 0:
                        ...
                        if X_train.shape[0] > 0 and X_test.shape[0] > 0:
                            try:
                                ...
                                # Graphique montrant les prédictions vs valeurs réelles
                                fig = go.Figure()
                                
                                # Ajouter les points réels vs prédits
                                fig.add_trace(go.Scatter(
                                    x=y_test,
                                    y=y_pred,
                                    mode='markers',
                                ...
                                # Ajouter une ligne diagonale pour la prédiction parfaite
